				...
				<li key={collection.name}>
					<div className={"collection-container"}>
						<div className={"topstory"}>
							<label htmlFor={collection.name + '_randomness'}>Casualit&agrave;: <span>{collection.randomness}</span></label>
							<input type="range"
								min="1" max="10"value={collection.randomness} step="1" 
								className={"slider"} name={collection.name + '_randomness'} id={collection.name + '_randomness'}
								onChange={() => { 
						...
						<li className={isEditing ? 'editing' : ''}>
							<div className={"story-header"}>
							   ...
							   {story.userStory && story.lastUpdate ? 
								<a href="#" onClick={() => {StoryAPI.restoreSystemStory(story)}}>
